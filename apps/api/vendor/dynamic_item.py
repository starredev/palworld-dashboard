# Palworld v1.0 dynamic-item format. Overlays palworld-save-tools v0.24.0's
# pre-1.0 module: since 1.0 the weapon/armor/egg payloads carry 4 leading bytes
# before the durability (and trailing bytes after). Without this overlay, any
# weapon/armor record we write is 4 bytes misaligned and the game reads garbage
# durability — items arrive "broken". Format mirrors the actively maintained
# community fork (deafdudecomputers/PalworldSaveTools, src/palsav).
...
def decode_bytes(
    ...
    if isinstance(egg_data, dict):
        ...
        try:
            ...
            # Unrecognised layout: keep the raw bytes so encode round-trips.
            data["type"] = "unknown"
...
def try_read_egg(reader: FArchiveReader) -> Optional[dict[str, Any]]:
    ...
    try:
        ...
        if e.args and e.args[0] == "Warning: EOF not reached":
            raise e
        reader.data.seek(cur_pos)
        return None
